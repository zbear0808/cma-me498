...
class GAlgClass(object):
    def __init__(self, pop_size, weights, min_weight, max_weight, n_mating, n_offspring, idx_crossover, pm, round_int):
        self.POP_SIZE = pop_size
        self.WEIGHTS = weights
        self.N_WEIGHTS = len(weights)
        self.MIN_WEIGHT = min_weight
        self.MAX_WEIGHT = max_weight
        self.N_MATING = n_mating
        self.N_OFFSPRING = n_offspring
        self.IDX_CROSSOVER = idx_crossover
    ...
    def hard_constraint(self, t_total_pop):
        ...
        """
        idx1 = (t_total_pop >= self.MIN_WEIGHT)
        idx2 = (t_total_pop <= self.MAX_WEIGHT)
    ...
    def graphPlot(self):
        ...
        curr_population = np.random.uniform(low=-4.0, high=4.0, size=self.DOFS_IN_POP)
        if self.ROUND_INT:
...
class Knapsack(GAlgClass):
    def __init__(self, pop_size, weights, min_weight, max_weight, n_mating, n_offspring, idx_crossover, pm, round_int, values):
        super().__init__(pop_size,weights, min_weight, max_weight, n_mating,n_offspring,idx_crossover,pm, True)
        self.VALUES = values
        
    def mutation(self, t_offspring):
        random_mutator = np.random.uniform(0.0, 1.0, (self.N_OFFSPRING,))

        idx = random_mutator > super().PM

        nnz = np.count_nonzero(idx)

        mutated_offspring = t_offspring.copy()

        mutated_offspring[idx] += np.random.uniform(-0.5, 0.5, (nnz, super().N_WEIGHTS))
    ...
    def calc_fitness(self, t_pop):
        ...
        """        
        # Do elementwise across columns, then sum the columns up together
        # fitness = np.sum(t_pop * VALUES, axis=1)

        # Do a matrix vector multiply of (10,6) * (6,)
        fitness = t_pop @ self.VALUES
...
def main():
    ...
    gAlg = GAlgClass(10,WEIGHTS, MIN_WEIGHT, MAX_WEIGHT, N_MATING,N_OFFSPRING,IDX_CROSSOVER,PM, False)
    gAlg.graphPlot()
main()